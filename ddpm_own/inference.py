...
sd = Diffusion(num_steps=1000, img_shape=(3, 32, 32), device=DEVICE)
model = UNet(IN_CHANNELS, OUT_CHANNELS, TIME_EMB_DIM, num_resnet_blocks=2).to(DEVICE)
dataset_loader = DatasetLoader(dataset_name="Flowers", batch_size=BATCH_SIZE, img_shape=(3, 32, 32), shuffle=True, device="cuda")

model.load_state_dict(torch.load('ddpm_own/model.pt'))
...
# plt.savefig('ddpm_own/outputs/ddpm_inference_img.png')

generated_images = sd.reverse_diffusion_ddim(num_steps = 650, num_images=8, model=model)
images = dataset_loader.inverse_transform(generated_images)
grid_img = make_grid(images / 255, nrow=4, padding=2)
plt.imshow(grid_img.permute(1, 2, 0).cpu().numpy())
plt.savefig('ddpm_own/outputs/ddim_inference_img.png')